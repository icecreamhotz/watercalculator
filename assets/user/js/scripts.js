...
$(function() {
	...
	$(".needs-validation-register").submit(function() {
		var form = $(this);
		let validate = form[0].checkValidity();
		let validateUsername = true;
		let validatePassword = true;
		let validateConfirmPassword = true;

		const regexUsername = /^[a-zA-Z0-9!@#\$%\^\&*\)\(+=._-]{6,}$/g;
		const username = $("#username").val();
		if (!regexUsername.test(username)) {
			$("#error-username").text(
				"Username must be not contain special character and more 6 characters."
			);
			$("#error-username").show();
			$("#username").addClass("invalid-important");
			validateUsername = false;
		} else {
			checkUsername(username).then(function(value) {
				validateUsername = value;
				if (validateUsername === false) {
					$("#error-username").text(
						"Username is exists, Please change your username."
					);
					$("#error-username").show();
					$("#username").addClass("invalid-important");
					validateUsername = true;
				} else {
					$("#username").removeClass("invalid-important");
					$("#error-username").hide();
				}
			});
		}
		...
		if (!regexPassword.test(password)) {
			$("#error-password").text(
				"Password must be contain One digit, One lower case, One upper case and least 8 from the mentioned characters."
			);
			$("#error-password").show();
			$("#password").addClass("invalid-important");
			$("#password2").addClass("invalid-important");
			$("#error-confirmpassword").hide();
			validatePassword = false;
		} else {
			validatePassword = true;
			$("#password").removeClass("invalid-important");
			$("#error-password").hide();
		}

		const password2 = $("#password2").val();
		if (password !== password2) {
			$("#error-password").text("Please input again password not match.");
			$("#password").addClass("invalid-important");
			$("#error-password").show();

			$("#error-confirmpassword").text(
				"Please input again password not match."
			);
			$("#password2").addClass("invalid-important");
			...
			if (validatePassword === true) {
				...
				$("#password2").removeClass("invalid-important");
				$("#error-confirmpassword").hide();
			}
		}
